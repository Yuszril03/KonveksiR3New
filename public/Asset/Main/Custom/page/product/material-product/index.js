...
document.getElementById('saveData').addEventListener('click', function () {
  let url = linkUrl + 'SaveDataMaterialProduct'
  let Data = {
  ...
  if (isEdit == 1) {
    ...
    url = linkUrl + 'UpdateDataMaterialProduct'
  }
...
$(document).on('click', '.nonaktif', function () {
  ...
  Swal.fire({
    title: 'Apakah anda yakin',
    text: 'Menonaktifkan bahan produk ini!',
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: linkUrl + '/C_Product/UpdateStatusMaterialProduct/' + id + '/0',
        dataType: 'JSON'
      }).done(hasil => {
        if (hasil == 1) {
          Swal.fire({
...
$(document).on('click', '.aktif', function () {
  ...
  Swal.fire({
    title: 'Apakah anda yakin',
    text: 'Mengaktifkan bahan produk ini!',
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: linkUrl + '/C_Product/UpdateStatusMaterialProduct/' + id + '/1',
        dataType: 'JSON'
      }).done(hasil => {
        if (hasil == 1) {
          Swal.fire({
