...
document.getElementById('Back1').addEventListener('click', function () {
  $.ajax({
    url: linkUrl + '/C_Transaction/closeTransUser',
    dataType: 'JSON'
  }).done(result => {})
...
let alertTransOpen = new bootstrap.Modal(
  document.getElementById('alertTransOpen'),
  {
    keyboard: false,
    backdrop: 'static',
    focus: true
...
let transIntervalOpen = setInterval(timerTrans, 1000)

function timerTrans () {
  $.ajax({
    url: linkUrl + '/C_Transaction/checkOpenTransUser',
    dataType: 'JSON'
  }).done(result => {
    // console.log(result)
    if (result == 0) {
...
function StopTrans () {
  clearInterval(transIntervalOpen)
  alertTransOpen.show()
}

document.getElementById('btnCloseTrans').addEventListener('click', function () {
  window.location.href = linkUrl + '/Riwayat-Transaksi'
})
...
function setDefaultTransaksi (type, data) {
  if (type == 1) {
    ...
    document.getElementById('simpantrans').disabled = true
    document.getElementById('simpantrans').classList.add('d-none')
...
function checkTransaction () {
  $.ajax({
    url: linkUrl + 'C_Transaction/SetTransactionEdit',
    // data: data,
    method: 'GET',
    dataType: 'JSON'
  }).done(result => {
...
document.getElementById('nextPay').addEventListener('click', function () {
  ...
  } else if (nominal < total[1].replaceAll('.', '')) {
    Swal.fire({
      title: 'Apakah anda yakin',
...
function sendToPayment (formData) {
  $.ajax({
    ...
    if (kondisi == 3) {
      ...
      //   cancelButtonText: 'Kembali ke Riwayat Transaksi',
      //   focusConfirm: true
      // }).then(result => {
      //   if (result.isConfirmed) {
