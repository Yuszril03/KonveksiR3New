...
function setData () {
  $.ajax({
    url: linkUrl + 'C_Transaction/getHistoryTrans',
    dataType: 'JSON',
    data: query,
    method: 'POST'
  }).done(result => {
...
function setList (pages, total) {
  ...
  for (let i = initial; i < batas; i++) {
    if (data[i] != null) {
      ...
      values += `<div class='shadow mt-3 mb-3 bg-body-tertiary container-list'>
      <div class='row'>
...
function setSpans (isFilter, spanSort, spanStatus, spanTanggal) {
  if (isFilter == 0) {
